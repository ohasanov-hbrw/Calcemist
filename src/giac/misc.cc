...
using namespace std;
#ifdef HAVE_UNISTD_H
#if !defined NUMWORKS && !defined HP39 && !defined HPG1
#include <dirent.h>
#endif
#include <sys/stat.h>
#include <sys/types.h>
...
#else
  const int pixel_lines=320;//=1024;
  const int pixel_cols =240;//=768;
#endif
#endif
#if defined GIAC_HAS_STO_38 || defined KHICAS || defined SDL_KHICAS 
...
static define_unary_function_eval (__variance,&_variance,_variance_s);
  ...
  static void find_xyfreq(const gen & g,gen & gv,int & xcol,int & ycol,int &freqcol,GIAC_CONTEXT){
	...
	freqcol=v[3].val;
      ...
      if (!gv._VECTptr->empty() && gv._VECTptr->front().type!=_VECT && gv._VECTptr->size()%2==0){
        vecteur & v=*gv._VECTptr;
        vecteur w; w.reserve(v.size()/2);
        for (int i=0;i<v.size();i+=2)
          w.push_back(makevecteur(v[i],v[i+1]));
        gv=w;
      }
...
#else      
      int c= 0;//pixel_buffer[y.val][x.val];
#endif
