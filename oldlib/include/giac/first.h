...
#undef printf
// 2. Enhanced Proxy Class: Handles duplicates, return types, AND memory pointer casting
struct stdostream {
    // Trick the compiler when it takes the address of os_cerr
    std::ostream* operator&() { return &std::cerr; }
    const std::ostream* operator&() const { return &std::cerr; }
    
    // Implicit conversion to standard reference
    operator std::ostream&() { return std::cerr; }

    // Intercept stream operations and return our own type to satisfy header signatures
    template<typename T>
    stdostream& operator<<(const T& val) {
        std::cerr << val;
        return *this;
    }
    stdostream& operator<<(std::ostream& (*manip)(std::ostream&)) {
        std::cerr << manip;
        return *this;
    }
};
